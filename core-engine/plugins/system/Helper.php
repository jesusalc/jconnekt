...
class JCHelper{
	...
	public static function isExAppEnabled($appID,$place=null){
		...
		$db=JFactory::getDBO();
		$sql='SELECT published FROM #__jc_exApps WHERE appID='.(int)$appID;
		$db->setQuery($sql);
		$res=$db->loadObject();
		if($db->getErrorNum()){
	...
	*/
	public static function getMeta($appName,$metaKey){
		...
		$sql='SELECT value FROM #__jc_meta' .
			' WHERE appID='.(int)$appID .' AND metakey='.$db->quote($metaKey);
		$db->setQuery($sql);
		$res=$db->loadObject();
		if($db->getErrorNum()){
			throw new Exception($db->getErrorNum() .' :: '. $db->getErrorMsg());
	...
	*/
	static function setMeta($appID,$metaKey,$value){
		...
		if(isset($meta)){
			//update
			$sql='UPDATE #__jc_meta SET value='.$db->quote($value).' WHERE appID='.(int)$appID.' AND metaKey='.$db->quote($metaKey);
			$db->Execute($sql);
		}
		else{
			//insert
			$sql='INSERT INTO #__jc_meta(appID,metaKey,value) VALUES '.
				'('.(int)$appID.','.$db->quote($metaKey).','.$db->quote($value).')';
			$db->Execute($sql);
		}
	...
	*/
	...
	public static function createJoomlaUser($username,$email,$password,$groupname){
		...
		$sql='INSERT INTO #__core_acl_aro(section_value,value,name) '.
					"VALUES ('users',".(int)$userID.','.$db->quote($username).')';
		$db->Execute($sql);
		if($db->getErrorMsg()) throw new Exception($db->getErrorMsg(),2345);

		$sql='SELECT id FROM #__core_acl_aro WHERE value='.$db->quote($userID);
		$db->setQuery($sql);
		$res=$db->loadObject();
		if($db->getErrorMsg()) throw new Exception($db->getErrorMsg(),2345);
	...
	public static function updateJoomlaUser($username,$email,$password,$groupname){
		...
		if($groupname) $sql.=', gid='.(int)$gid.', usertype='.$db->quote($groupname);
		...
		if($groupname){
			...
			if($db->getErrorMsg()) throw new Exception($db->getErrorMsg(),2345);
			$sql='UPDATE #__core_acl_groups_aro_map SET group_id='.(int)$gid.' WHERE aro_id='.(int)$res->id;
			$db->Execute($sql);
			if($db->getErrorMsg()) throw new Exception($db->getErrorMsg(),2345);
		}
	}
